...
import { Bus, ArrowRight, Search, ArrowLeftRight, ShieldCheck } from 'lucide-react';
import { searchBuses, BUS_CITIES } from '@/data/buses';
import { getTomorrowString } from '@/lib/utils';
import BusCard from '@/components/search/BusCard';
import FilterSidebar from '@/components/search/FilterSidebar';

function BusSearchInner() {
  ...
  const filteredResults = results.filter(bus => {
    if (filters.maxPrice && bus.baseFare > filters.maxPrice) return false;
    if (filters.times?.length) {
      const [h] = bus.departure.split(':').map(Number);
  ...
  return (
    <div className="min-h-screen bg-slate-50 pt-16">
      {/* Search bar */}
      <div className="bg-gradient-to-r from-slate-900 to-amber-900">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
          <div className="flex items-center gap-2 text-white/70 text-sm mb-4">
            <Bus className="w-4 h-4" /><span>Bus Search</span>
          </div>
          ...
          <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 border border-white/20">
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <div className="flex-1">
                <select value={to} onChange={e => setTo(e.target.value)} className="input-base">
                  ...
                  {BUS_CITIES.filter(c => c !== from).map(city => (
                    <option key={city} value={city}>{city}</option>
                  ))}
                </select>
              </div>
            ...
            {from && to && (
              <div className="mt-3 flex items-center gap-2 text-xs text-amber-200/80">
                ...
                <span>Cross-border routes require valid travel documents. Check visa requirements before booking.</span>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        {!searched ? (
          <div className="text-center py-24 text-slate-400">
            ...
            <p className="text-lg font-medium">Search for buses above to get started</p>
            <p className="text-sm mt-2 opacity-70">International bus routes across South Asia</p>
          </div>
        ) : (
          <div className="flex gap-6">
            <div className="hidden lg:block w-64 shrink-0">
              <FilterSidebar filters={filters} onChange={setFilters} type="buses" />
            </div>
            <div className="flex-1 space-y-4">
              <div className="flex items-center justify-between">
                <p className="text-slate-600 text-sm">
                  {filteredResults.length > 0 ? (
                    <>
                      <span className="font-bold text-slate-900">{filteredResults.length}</span> buses from{' '}
              ...
              {filteredResults.length === 0 ? (
                <div className="bg-white rounded-2xl border border-slate-100 p-16 text-center">
                  <Bus className="w-12 h-12 mx-auto mb-4 text-slate-200" />
